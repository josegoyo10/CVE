 ...
 *                    Kristian Koehntopp
 *
 * $Id: template.class.php,v 1.2 2008/10/28 14:34:41 jlopez Exp $
 *
 */
...
class template{
  ...
  */
  function subst($varname) {
    ...
    //Modificaci�n RGM 24/03/2003. Debemos buscar si existe alguna aparici�n de una variable del estilo {varname(N)} y si es as�, tratarla de forma especial para rellenar
    $nstr = $str;
    while ($nstr) {
        if (ereg("(\{([a-zA-Z0-9_]+)\(([0-9]+)\)\((i|d)\)\})", $nstr, $regs)) {
            //$regs[1] => Toda la coincidencia, $regs[2] => Nombre variable, $regs[3] => relleno de campo, $regs[4] => Alineaci�n

            $str = str_replace($regs[1], "{".$regs[2]."}", $str);
            if ($regs[4] == "d") {
                $this->varvals[$regs[2]] = sprintf("%".$regs[3]."s", $this->varvals[$regs[2]]);
            }elseif ($regs[4] == "i") {
                $this->varvals[$regs[2]] = sprintf("%-".$regs[3].".".$regs[3]."s", $this->varvals[$regs[2]]);
            }

            $nstr = substr($nstr, strpos($nstr, $regs[1])+strlen($regs[1]));
        }
        else {
