    ...
    public function getreportecuadratura($List) {
		...
		$query="SELECT 	d.numdocumento,
				...
				FROM documento_e d
					...
					join ordenent_e oe on oe.id_ordenent = d.numorigen and d.tipoorigen = 'OE' and oe.id_estado in ('OA', 'OB', 'OM', 'OF', 'OG', 'OR')
...
/* Reporte Documentos area tributaria*/
	...
	public function getreportecotizacion($List) {
			...
			$query="SELECT c.id_cotizacion,
						...
						c.codigovendedor codigovendedor,
						IF(concat(us.usr_nombres,' ', us.usr_apellidos) IS NOT NULL,concat(us.usr_nombres,' ',us.usr_apellidos),'Sin Apellido o Nombre') apellidoynombre,
                                                de.numdocumento AS Guia,
                                                de.numdocrefop AS Nro_OP
                                            FROM
                                                cotizacion_e c
                                                LEFT JOIN
                                                ordenent_e oe ON c.id_cotizacion = oe.id_cotizacion
                                                LEFT JOIN
                                                documento_e de ON oe.id_ordenent = de.numorigen
                                                left join 
                                                usuarios us on c.codigovendedor = us.codigovendedor
						left join 
                                                estado e on e.id_estado = c.id_estado and e.tipo = 'CO'
						left join 
                                                tipoventa t on t.id_tipoventa = c.id_tipoventa
						left join 
                                                locales l1 on c.codlocalventa = l1.cod_local
						left join 
                                                locales l2 on c.codlocalventa = l2.cod_local
						left join 
                                                cliente i on c.rutcliente = i.rut
...
//		die ($query);	
	        ...
	        while ($row = $res->fetch_assoc()){
				...
				$Registro->nomlocemi		=	$row['localemisor'];
                                $Registro->codigovendedor	=	$row['codigovendedor'];	
 		 	 	$Registro->apellidoynombre	=	$row['apellidoynombre'];   
				$Registro->nomloccsum		=	$row['localsuministro'];								
				$Registro->total_neto		=	$row['valortotal'];
                                $Registro->Guia                 =	$row['Guia'];//Mantis 27362 inicio
                                $Registro->Nro_OP		=	$row['Nro_OP'];//Mantis 27362 inicio
	            $List->addlast($Registro);
	        }
	        $res->free();
	        return true;
		...
		public function getreportedesbloqueoscotizacion($List) {
			$List->gofirst();
	$query="SELECT  des.id_cotizacion,
                        DATE_FORMAT(c.feccrea, '%d-%m-%Y') fcreacion,
                        c.usuariocrea,
                        e.descripcion estado,
                        c.rutcliente,
                        ...
                        ROUND(c.valortotal) valortotal,
                        c.margentotal,
                        des.motivo motivodesbloqueo,
                        des.descripcion comentariodesbloqueo,		
                        DATE_FORMAT(des.fecha, '%e/%m/%Y %H:%i:%S')  fechadesbloqueo,
                        IF(B.usr_login IS NOT NULL,B.usr_login,'Usuario no encontrado') as Usuario
                        
                FROM desbloqueoscoti des
		left join cotizacion_e c on des.id_cotizacion = c.id_cotizacion 
		left join usuarios us on c.codigovendedor = us.codigovendedor
		...
		left join cliente i on c.rutcliente = i.rut
                LEFT JOIN
                usuarios B ON LOWER(c.id_usuario) = LOWER(B.usr_id)
		where 1 
			" . (($List->getelem()->fecinicio)? " and c.feccrea >= '".$List->getelem()->fecinicio."'" : "") . "
			" . (($List->getelem()->fectermino)? " and c.feccrea <= '".$List->getelem()->fectermino."'" : "") . "
			" . (($List->getelem()->fechadesbloqueo1)? " and des.fecha >= '".$List->getelem()->fechadesbloqueo1."'" : "") . "
			" . (($List->getelem()->fechadesbloqueo2)? " and des.fecha <= '".$List->getelem()->fechadesbloqueo2."'" : "") . "
			" . (($List->getelem()->cotizacion)?" and c.id_cotizacion = '".$List->getelem()->cotizacion."' " : "") . "
			" . (($List->getelem()->rutcliente)?" and c.rutcliente = ".$List->getelem()->rutcliente." " : "") . "
			" . (($List->getelem()->razonsoc)? " and c.razonsoc like '%".$List->getelem()->razonsoc."%'" : "") . "
			" . (($List->getelem()->tipo_venta)?" and c.id_tipoventa = '".$List->getelem()->tipo_venta."' " : "") . "
			...
			" . (($List->getelem()->codlocalcsum)?" and c.codlocalcsum = '".$List->getelem()->codlocalcsum."' " : "") . "
			" . (($List->getelem()->estado)?" and c.id_estado = '".$List->getelem()->estado."' " : "") . "
			" . (($List->getelem()->motivodesbloqueo)?" and des.motivo LIKE '%".$List->getelem()->motivodesbloqueo."%' " : "") . "
		...
		//echo $query;
	        ...
	        while ($row = $res->fetch_assoc()){
			    ...
			    $Registro->codigovendedor	=	$row['codigovendedor'];	
 		 	 	$Registro->apellidoynombre	=	$row['apellidoynombre'];   				
				$Registro->total_neto		=	$row['valortotal'];	
				$Registro->fechadesbloqueo		=	$row['fechadesbloqueo'];		
				$Registro->motivodesbloqueo		=	$row['motivodesbloqueo'];		
				$Registro->comentariodesbloqueo		=	$row['comentariodesbloqueo'];
                                $Registro->Usuario		=	$row['Usuario'];//Mantis 27363
	...
	$query="SELECT  des.id_cotizacion,
		...
		left join locales l1 on c.codlocalventa = l1.cod_local
		left join cliente i on c.rutcliente = i.rut
                left join usuarios B ON LOWER(c.id_usuario) = LOWER(B.usr_id)
		where 1 
			" . (($List->getelem()->fecinicio)? " and c.feccrea >= '".$List->getelem()->fecinicio."'" : "") . "
			" . (($List->getelem()->fectermino)? " and c.feccrea <= '".$List->getelem()->fectermino."'" : "") . "
			" . (($List->getelem()->fechadesbloqueo1)? " and des.fecha >= '".$List->getelem()->fechadesbloqueo1."'" : "") . "
    ...
    if (!$res) throw new DAOException(__CLASS__ , __FUNCTION__ , $this->bd->error(), $query, 1);
//mantis 27363
    $fp = "Cotizacion\tF. Creacion\tEstado\tCC/NIT/RUT\tRazon Social\tLocal de Venta\tUsuario\tNombre y Apellido\tCodigo Vendedor\tTotal Neto\tMargen\tFecha Desbloqueo\tMotivo Desbloqueo\tComentario Desbloqueo\n";

       while($row = $res->fetch_assoc()) {
	   		$fp .=	$row['id_cotizacion']."\t";
			$fp .=	$row['fcreacion']."\t";
			$fp .=$row['estado']."\t";
			$fp .=$row['rutcliente']."\t";
			$fp .=	$row['razonsoc']."\t";
			$fp .=	$row['localemisor']."\t";
                        $fp .=$row['Usuario']."\t";//mantis 27363
			$fp .=$row['apellidoynombre']."\t";   	
			$fp .=	$row['codigovendedor']."\t";			
			$fp .=	$row['valortotal']."\t";	
			$fp .=	$row['margentotal']."\t";
			$fp .=$row['fechadesbloqueo']."\t";		
			$fp .=	$row['motivodesbloqueo']."\t";		
			$fp .=	$row['comentariodesbloqueo']."\t";
        ...
        header( 'Content-Disposition: attachment;filename=reporte_desbloqueo.xls');
        ...
        /* Reporte cotizaciones Excel maxi*/
...
//    die ($query);                                    
    ...
    $fp = "Cotizacion\tUsuario\tF. Creacion\tF. Vencimiento\tEstado\tCC/NIT/RUT\tRazon Social\tTipo Venta\tLocal Emisor\tTienda\tCodigo Vendedor\tNombre y Apellido\tTotal Neto\tMargen Total\tGuia\tNro_op\n";//Mantis 27362 inicio

       while($row = $res->fetch_assoc()) {
            $fp .= $row['id_cotizacion']."\t";
            $fp	.= $row['usuariocrea']."\t"; 
            $fp .= date("d/m/Y", strtotime( $row['fcreacion']) )."\t";
            $fp	.= date("d/m/Y", strtotime( $row['fvencimiento']) )."\t";
            $fp	.= $row['estado']."\t"; 
            $fp	.= $row['rutcliente']."\t";  
            $fp	.= $row['razonsoc']."\t";
            $fp	.= $row['tipoventa']."\t";
            $fp	.= $row['localemisor']."\t";
            $fp	.= $row['localsuministro']."\t";
            $fp	.= $row['codigovendedor']."\t";
            $fp	.= $row['apellidoynombre']."\t";
            $fp	.= $row['valortotal']."\t";
            $fp	.= $row['margentotal']."\t";
            $fp	.= $row['Guia']."\t";//Mantis 27362 inicio
            $fp	.= $row['Nro_OP']."\t";//Mantis 27362 inicio
            $fp .= "\n";
       }
       
        header( 'Content-Type: application/vnd.ms-excel' );
        header("Content-Length: ".strlen($fp));
        header( 'Content-Disposition: attachment;filename=reporte_guia_despacho.xls');
	...
	public function getventascliente($List) {
			$List->gofirst();
			$condicionFolio = '';
			if($List->getelem()->tipofolio == 'noemitido'){
				$condicionFolio = " and length(numdocumento) < 14";
			}
			if($List->getelem()->tipofolio == 'emitido'){
				$condicionFolio = " and length(numdocumento) >= 14";
			}
                        //Mantis 26781
                        // en la linea " . (($List->getelem()->codigovendedor)? " and o.codigovendedor= '".$List->getelem()->codigovendedor."'" : "") . "
                        // se agregan las comillas simples
                       	//(round((((sum(od.totallinea)-(sum(od.pcosto* od.cantidade)))* 100)/sum(od.totallinea)) * 100)/100) as margen,
			...
			$query="SELECT 	o.fechacompra,
							...
							sum(od.totallinea) as totaldet,
              				o.totaloe as total,
              				CASE 
								WHEN (o.margen IS NULL) THEN  (round((((sum(od.totallinea)-(sum(od.pcosto* od.cantidade)))* 100)/sum(od.totallinea)) * 100)/100)
									ELSE
					             o.margen
					        END AS margen,
              				id_contribuyente,
          ...
          file_put_contents("queryReporteMargen.txt",print_r($query,true));
	...
	public function getReporteTransferenciaMercancia($List) {
        ...
        $query = "	SELECT
                            ...
                            oe.feccrea,
                            codlocalventa,
                            lf.nom_local nom_codlocalventa,
                            codlocalcsum,
                    ...
                    LEFT JOIN 	locales lf on lf.cod_local=oe.codlocalventa
					LEFT JOIN 	tipoentrega te on te.id_tipoentrega = oe.id_tipoentrega
                    WHERE 	1 and oe.codlocalventa <> oe.codlocalcsum
                    " . (($List->getelem()->codigovendedor)? " and oe.codigovendedor= '".$List->getelem()->codigovendedor."' " : "") . "
                    ...
                    " . (($List->getelem()->fecha_ini)? " and oe.fechacompra >= '".$List->getelem()->fecha_ini."'" : "") . "
                    " . (($List->getelem()->fecha_fin)? " and oe.fechacompra <= '".$List->getelem()->fecha_fin."'" : "") . "
					...
					" . (($List->getelem()->codlocalfactura)? " and codlocalventa = '".$List->getelem()->codlocalfactura."' " : "") . "
					" . (($List->getelem()->codlocalsuministro)? " and codlocalcsum = '".$List->getelem()->codlocalsuministro."' " : "") . "
        ...
        $List->clearlist();
        while ($row = $res->fetch_assoc()){
            $Registro = new dtotransferenciamercancia;
            $Registro->id_ordenent				= 	$row['id_ordenent'];
            $Registro->id_cotizacion			= 	$row['id_cotizacion'];
            $Registro->id_estado				= 	$row['id_estado'];
            $Registro->estado					= 	$row['nomestadorent'];	
            $Registro->numdocpago				= 	$row['numdocpago'];
            $Registro->tipoentrega				= 	$row['nomtipoentrega'];
            $Registro->rutcliente				= 	$row['rutcliente'];
            $Registro->localfactura				= 	$row['nom_codlocalventa'];
            $Registro->localsuministro			= 	$row['nom_localcsum'];
            $Registro->razonsoc					= 	$row['razonsoc'];
	...
	public function setdesbloqueo($List){
        $query = "	INSERT INTO desbloqueoscoti (	
                        id_cotizacion,
						usrcrea,
						motivo,
                        descripcion,
    ...
    public function getModulos($List){
        $List->gofirst();
        $query = "SELECT MOD_ID, MOD_PADRE_ID, MOD_ESTADO, MOD_NOMBRE, MOD_DESCRIPCION, "
                . "MOD_URL, MOD_ORDEN, MOD_USR_CREA, MOD_FEC_CREA, MOD_USR_MOD, MOD_FEC_MOD FROM modulos";
    ...
    public function getPermisosXModulo($List){
        $List->gofirst();
        $query = "SELECT PEMO_MOD_ID, PEMO_TIPO, PEMO_PER_ID, PEMO_INSERT, PEMO_DELETE, "
                . "PEMO_UPDATE, PEMO_SELECT, PEMO_USR_CREA, PEMO_FEC_CREA, PEMO_USR_MOD, PEMO_FEC_MOD FROM permisosxmodulo";
    ...
    //Mantis 26818 Inicio
    //Mantis 27281- se aplicaca el replace a los campos String Inico
    public function getClientesAsignadosVendedores($List, &$array = null)
    {
        $List->gofirst();
        $query = "SELECT 
                    replace(c.razonsoc,';','') AS Razon_Social,
                    c.RUT,
                    replace(d.descripcion,';','') AS TipoCliente,
                    c.codigovendedor,
                    REPLACE(u.usr_nombres, ';', '') AS Nombre_Vendor,
                    REPLACE(u.usr_apellidos, ';', '') AS Apellido_Vendor,
                    REPLACE(c.contacto, ';', '') AS contacto,
                    c.fonocontacto as fonocontacto,
                    replace(c.email,';','') as email,
                    replace(c.direccion,';','') as direccion,
                    c.id_contribuyente AS Tipo_Contribuyente,
                    c.id_profesion AS profesion
                FROM
                    cliente c
                        JOIN
                    tipocliente d ON c.id_tipocliente = d.id_tipocliente
                                JOIN
                    usuarios u on c.codigovendedor=u.codigovendedor";
   ...
   //JOSE GREGORIO TOTALES ASIGNADOS.

       public function getClientesTotalesAsignados($List, &$array = null)
    {
        $List->gofirst();
        $query = "SELECT 
                    replace(c.razonsoc,';','') AS Razon_Social,
                    ...
                    replace(d.descripcion,';','') AS TipoCliente,
                     c.codigovendedor,
                    concat(u.usr_nombres,' ',u.usr_apellidos) AS NombreCompleto,
                     REPLACE(c.contacto, ';', '') AS contacto,
                    c.fonocontacto as fonocontacto,
                    replace(c.email,';','') as email,
                    replace(c.direccion,';','') as direccion,
                    t.descripcion as Tipo_Contribuyente, 
           			x.descripcion as profesion
                FROM 	cliente c, tipocliente d , 
                		 tipocontribuyente t, 
                		 cu_profesion x,usuarios u
                     WHERE      c.id_tipocliente = d.id_tipocliente 
							AND  c.id_contribuyente = t.id_contribuyente 
        					AND  c.id_profesion = x.id
        					AND  c.codigovendedor = u.codigovendedor;";

		//         $query = "SELECT * FROM os o
		// JOIN historial h ON o.id_os = h.id_os
		// JOIN locales l ON l.id_local = o.id_local
		// JOIN os_detalle osde ON o.id_os = osde.id_os
		// WHERE hist_descripcion LIKE 'Cotización ha sido pagada%'
		// and os_fechacreacion between '2018-09-01' and '2018-09-25';";
      ...
      //JOSE GREGORIO Reporte de clientes sin compra dada una fecha de inicio y fin.
     // 27-02-2019
     public function getClientesnocompra($List,$fecha_inicio, $fecha_fin,$codigo_vendedor,&$array = null)
    	{
        	
        	$List->gofirst();

           //Llamada a el Store Procedure para Obtener los usuarios que nos han comprado.

            $query_SP = 'CALL getUsuariosInfo("'.$codigo_vendedor.'","'.$fecha_inicio.'","'.$fecha_fin.'")';
            
              $res_SP = $this->bd->query($query_SP);
		         if(!$res_SP){
		            throw new DAOException(__CLASS__, __FUNCTION__, $this->bd->error(), $query, 1);
		         }
         //file_put_contents('fechaIniClte.txt', print_r($query,true));

		  $query = "SELECT *FROM usuarios_temporal";
     ...
     //JOSE GREGORIO Reporte de clientes con compra dada una fecha de inicio y fin.
     // 01-03-2019
     public function getClientescompra($List,$fecha_inicio, $fecha_fin,$codigo_vendedor,&$array = null)
    	{
            ...
            $query = "SELECT
					   oe.id_ordenent,
					   c.rut,
					   REPLACE(c.razonsoc, ';', '') as nombre,
					   CASE WHEN c.celcontactoe =  0 THEN 'No Registrado' ELSE c.celcontactoe END as celular,
					   c.direccion,
     				   c.email as correo,
     				   c.fax,
     				   ...
     				   p.descripcion as profesion,
      				   c.codigovendedor
   					FROM ordenent_e oe
   					 JOIN cliente c ON c.rut = oe.rutcliente
   					 JOIN cu_profesion p ON p.id = c.id_profesion
        			 JOIN tipocontribuyente tc ON tc.id_contribuyente = c.id_contribuyente

				WHERE c.codigovendedor = '".$codigo_vendedor."' AND 
				oe.fechacompra between '".$fecha_inicio."' AND '".$fecha_fin."' LIMIT 2000 ";
   ...
   //truncate tabla.
    public function truncatetabla($nombre_tabla)
     {
    
            $query = "TRUNCATE TABLE ".$nombre_tabla." ";

             // file_put_contents('truncate.txt', $query);
    ...
    //Mantis 26818 Fin
    //Mantis 26826 Inicio
    public function getMedioPagoEntregaVenta($List, &$array = null)
    {
        
        $List->gofirst();
        $query = "SELECT 
                    replace(a.id_tipopago,';','') AS `id Tipo Pago`,
                    tp.descripcion AS `Tipo Pago`,
                    a.codlocalventa AS `Local venta`,
                    a.codigovendedor AS `Codigo vendedor`,
                    REPLACE(b.usr_nombres, ';', '') AS `Nombre Vendedor`,
                    REPLACE(b.usr_apellidos, ';', '') AS `Apellido Vendedor`,
                    a.rutcliente AS `RUT Cliente`,
                    a.id_ordenent AS `Orden de Entrega`,
                    a.numdocpago AS `Numero Documento`,
                    a.fechacompra AS `Fecha de Compra`,
                    a.indmsgsap,
                    a.totaloe AS `Total Orden de Entrega`,
                    a.totaliva AS `Total con IVA`
                FROM
                    ordenent_e a
                        LEFT JOIN
                    usuarios b ON a.codigovendedor = b.codigovendedor
                        LEFT JOIN
                    tipopago tp ON a.id_tipopago = tp.id_tipopago
                WHERE
                    fechacompra BETWEEN '".$List->getelem()->fecinicio."' and '".$List->getelem()->fectermino."'";
