...
class daoproductocpe{
	...
	public function getproducto($List){
        ...
        $query="SELECT	p.id_producto,
                ...
                AND b.unid_med in (".UNIMED_VALID.") AND p.estadoactivo <>'E'  AND cod_ppal = 1 AND b.estadoactivo <>'E' ORDER BY unidmed DESC LIMIT ".($List->getelem()->numretlimitdes+0).", ".($List->getelem()->numretlimit+0)." ;
                ";


       // file_put_contents("QueryProductos.txt", $query);
       
      // echo "SQL:".$sql."<br>\n";


    	general::writeevent('query productos B'. $query);
	...
	public function getproductof($List){
       ...
       // file_put_contents('getproductof.txt', $query);
    	//general::writeevent('query productos '. $query);
        $pag = $List->getelem()->pagactual+0;
        $res = $this->bd->query($query);
    ...
    public function getproductogrilla($List){
       ...
       // file_put_contents("getProducto.txt",print_r($List,true));
        ...
        $query="SELECT	Distinct
                ...
                LEFT JOIN codbarra b on b.cod_prod1 = p.cod_prod1
                LEFT JOIN prodxprov x on x.cod_prod1 = p.cod_prod1
                LEFT JOIN	proveedores v on v.cod_prov = x.cod_prov " . (($List->getelem()->nomprov)? " AND v.nom_prov like '%".str_replace(' ', '%', trim($List->getelem()->nomprov))."%' " : "") . "
                ...
                ";
        //general::writeevent("daoprodcuto grilla".$List->getelem()->actualizarinventario);
        $actualizarinventario=$List->getelem()->actualizarinventario;
        
         //file_put_contents("queryPRODCVE.txt", $query);

        $res = $this->bd->query($query);


		//general::writeevent("CONSULTA PRECIO: A" . $query);
        if (!$res) throw new DAOException(__CLASS__ , __FUNCTION__ , $this->bd->error(), $query, 1);

        $List->clearlist();
        while ($row = $res->fetch_assoc()){
            //file_put_contents("queryresPRODCVE.txt", print_r($row,true));
...
//		general::writeevent($query);
    ...
    public function getnivel1($List){
     ...
     /*Metodo para Obtener el Inventario Real */
    public function getproductoinventario($List){
    	$totalregsolic = $List->getelem()->numretlimit+0;
		$List->gofirst();
		$query="SELECT 	 
				P.cod_prod1 AS cod_sap,  
				P.des_larga AS producto, 
				PR.prec_valor AS precio, 
				L.nom_local AS tienda, 
				L.cod_local, 
				C.cod_barra AS ean   
                FROM 	productos P
                ...
                WHERE 1   AND PR.estadoactivo = 'C' AND P.estadoactivo = 'C' AND C.estadoactivo='C'
                " . (($List->getelem()->sap)? " AND C.cod_ppal = 1  AND P.cod_prod1 = '".$List->getelem()->sap."' " : "") . "
                " . (($List->getelem()->barra)? " AND C.cod_barra = '".$List->getelem()->barra."' " : "") . "
                " . (($List->getelem()->csum)? " AND L.cod_local = '".$List->getelem()->csum."' " : "") . "
                ";
        ...
        while ($row = $res->fetch_assoc()){
            $Registro = new dtoproducto;
            $Registro->sap   		= $row['cod_sap'];
            $Registro->descripcion	= $row['producto'];
            $Registro->pventa  		= $row['precio'];
            $Registro->descripcionc	= $row['tienda'];
            $Registro->csum   		= $row['cod_local'];
            $Registro->barra   		= $row['ean'];            
            $List ->addlast($Registro);
        }
        $res->free();
        return true;
    }
    /*Fin Metodo para Obtener el Inventario Real */
    
    //obtener descripciones para los sku
    public function getDescrSKU(&$list){ //$list:[<sku, text>]
        $in = '';
        $i  = 0;
        @$l2 = array_keys($list);
        do{
        ...
        $runico = false; //si no hay resultados, false
        while($row    = $res->fetch_assoc()){
            $list[$row['sap']] = $row['descripcion'];
            $runico            = Array($row['sap'] => Array('des' => $row['descripcion'],
                    'cat' => $row['categoria'])); //actualiza el resultado a devolver. Si es �nico, puedo retornar una descripci�n
        }
    ...
    public function getSkuDescr($descr){//obtiene codigo desde descripci�n
        $query = "SELECT "
                . "p.cod_prod1 sap, "
                . "p.des_larga descripcion "
                . "FROM "
