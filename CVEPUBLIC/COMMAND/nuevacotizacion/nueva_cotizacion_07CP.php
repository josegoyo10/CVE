...
//general::alert('rete ica'.$Listclien->getelem()->rete_ica.'rete renta'.$Listclien->getelem()->rete_renta);
	...
	if($cantidad_cienr != 0){
		...
		bizcve::getproductof($Listp = new connlist(new dtoproducto(array('sap'=>'539'))));
		$Listp->gofirst();
		...
		bizcve::getproductof($Listp = new connlist(new dtoproducto(array('sap'=>'540'))));
		$Listp->gofirst();
		...
		if($cantidad_diesr != 0){
			...
			/*Fin Costo de Cotizacion Actual*/
			bizcve::getproductof($Listp = new connlist(new dtoproducto(array('sap'=>'541'))));
			$Listp->gofirst();
		...
		if($cantidad_cienmr != 0){
			...
			bizcve::getproductof($Listp = new connlist(new dtoproducto(array('sap'=>'542'))));
			$Listp->gofirst();
		...
		echo " window.open('nueva_cotizacion_05.php?popup=1&aci=1&fchr=$fchdr&fch=$fchdp&id_cotizacion=$id_cotizacion','','top=1, left=100 ,width=800,height=810');";
		echo " window.close();"; 
		echo "</script>";
