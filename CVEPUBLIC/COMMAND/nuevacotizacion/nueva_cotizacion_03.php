...
if ($_POST['accion'] == 'grabar') {
	...
	//echo "obs:".$_POST['observacion']."<br>\n";
	
	$listaEnc = new connlist;
	$encCotizacion = new dtocotizacion;
	$encCotizacion ->id_cotizacion  =$_POST['id_cotizacion'];
	$fecha_hoy = date("d/m/Y") ;
	//echo "desde ".$fecha_hoy." hasta ".$fecha_hoy ;
	$encCotizacion ->validdesde 	  =general::formato_fecha_FORM2DB($fecha_hoy);
	...
	$encCotizacion ->observaciones_pos = $_POST['observacion_pos']; 
    ...
    // file_put_contents("enc.txt",$listaEnc);
    /*añadido J.G 06-02-2019 PARA AÑADIR OBSERVACION */
   // bizcve::actualizar_obsPos($encCotizacion->id_cotizacion,$encCotizacion->observaciones_pos);
...
/****** MODIFICACION GOA 27-12-2007. Desde ahora el pventa+iva será guardo en la base de datos*****/
	...
	if (!bizcve::putcotizacion($listaEnc,$listaDet)) {
		...
		$ListEncSoc = new connlist;
		$RegSoc= new dtocotizacion;
		$RegSoc->razonsoc=$_POST['razon_soc'];
		$RegSoc->id_cotizacion=$_POST['id_cotizacion'];
		$ListEncSoc->addlast($RegSoc);
		//bizcve::cambiar_razon_socio($ListEncSoc);
	...
	if($_POST['bloqueopormargen']==1) {
		...
		if (!bizcve::cambioestadocotizacion($listaEncCB)) {
			...
			bizcve::setevento(41, 'Modulo Cotizaciones', $_SERVER['REMOTE_ADDR'], 'ABM cotizacion',
                    'Orden de entrega numero '.$_POST['id_cotizacion'].' ha sido bloqueada','','Orden de entrega ha sido bloqueada', $nombreSession);
...
if($_GET['editar']=='Edit'){
	$listaEncVen = new connlist;
	$Cotiven = new dtocotizacion;
	$Cotiven ->id_cotizacion  =$_GET['id_cotizacion'];
	$listaEncVen->addlast($Cotiven);
	bizcve::cotizaciones_vencidas($listaEncVen);
	$listaEncVen-> gofirst();
	if($listaEncVen->getelem()->id_estado=="CD"){
		general::inserta_tracking($_GET['id_cotizacion'],"","","","La cotizacion a cambiado a estado Caducada");
		header("Location: nueva_cotizacion_04.php?id_cotizacion=" . $_GET['id_cotizacion']);	
	}
...
$cotizacion->actualizarinventario=true;
...
$listaEnc -> gofirst();
if(trim($listaEnc->getelem()->codlocalventa) != trim($ses_usr_codlocal) ){
?>
<script type="text/javascript">
alert('Centro de facturación diferente de local actual.')
window.location = window.location + '&np=1';
</script>
<?
	//exit();
header("Location: ./nueva_cotizacion_04.php?id_cotizacion=".$listaEnc->getelem()->id_cotizacion);
}
//if (($listaEnc->getelem()->codlocalventa != $ses_usr_codlocal) && $_REQUEST['np']) {
//	header("Location: ./nueva_cotizacion_04.php?id_cotizacion=".$listaEnc->getelem()->id_cotizacion);
//}
 /* 
 */
...
bizcve::getlocales($ListLoc = new connlist(new dtolocal(array('cod_local' => $listaEnc->getelem()->codlocalcsum))));
$ListLoc->gofirst();
...
do {
	array_push($productos_input, $listaDet->getelem()->codprod);
	array_push($productos_input_unimed, $listaDet->getelem()->unimed);
} while ($listaDet->gonext());

$centro_id = $listaEnc->getelem()->codlocalcsum;
$almacen_id = $ListLoc->getelem()->almacen_cod; // obtengo el codigo de almacen en base al codigo de local
...
// Vuelvo a cargar los productos luego de actualizar el stock
$listaEnc = new connlist();
$listaDet = new connlist();
$listaEnc->addfirst($cotizacion);
bizcve::getcotizacion($listaEnc, $listaDet);
...
if(!$credito = ConsultarClienteOnline($listaCliente->getelem()->rut)) {
	$disponible = bizcve::getdisponible($listaCliente);
}
else {
	$disponible = $credito['limite_disponible'];
	if (!($credito['limite_disponible'])) {
		$disponible = bizcve::getdisponible($listaCliente);
	}
}
...
$MiTemplate ->set_var("razon_soc_cli",$listaCliente->getelem()->razonsoc);
$MiTemplate ->set_var("direccion_dir_des",$listaEnc->getelem()->direccion);

$obra=explode("||",$listaEnc->getelem()->direccion);
$MiTemplate ->set_var("titulo_nom_dir",($obra[1]!=''?'Nombre Obra:':'Nombre Direcci&oacute;n:'));
...
if ($listaDet->numelem()){
	$MiTemplate->set_block('main' , "detalle" , "BLO_detalle");
		...
		do{
			...
			if (isset($productos[$listaDet->getelem()->barra])) {
				$stock = $productos[$listaDet->getelem()->barra];
			}
			else {  

				$stock = $listaDet->getelem()->lisdetprod->getelem()->stock;

			} 			
			
			$MiTemplate->set_var('grupocat', $listaDet->getelem()->lisdetprod->getelem()->grupocat);
			...
			do{				
				...
				$margen = Array();
                if($margen = bizcve::getMargenProd($listaDet->getelem()->lisdetprod->getelem()->sap)){//si, es una asignación.
                    $MiTemplate->set_var("margenminimo", $margen['margen']);
                }elseif($margen = bizcve::getDatosMargen($listaDet->getelem()->lisdetprod->getelem()->id_catprod)){
                    $MiTemplate->set_var("margenminimo", $margen['margen']);
                }else{
                    $MiTemplate->set_var("margenminimo", MARGEN_MINIMO);
                }

				
				// Si se pudo obtener el stock online lo traigo, caso contrario busco el del dia anterior
				if (isset($productos[$listaDet->getelem()->lisdetprod->getelem()->barra])) { 
					$stock = $productos[$listaDet->getelem()->lisdetprod->getelem()->barra];
				}
		        else {
		         	$stock = $listaDet->getelem()->lisdetprod->getelem()->stock;
		        }
				
				$MiTemplate ->set_var("stock1",$stock);
		...
		if ($recargo){
				...
				if ($lista->numelem()==1){
					...
					$MiTemplate ->set_var("stock2",$stock);
