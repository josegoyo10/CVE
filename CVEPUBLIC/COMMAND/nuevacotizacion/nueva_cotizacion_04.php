...
if ($accion == 'Duplicar') {
	...
	$listaEncCD->addlast($CotizacionCD);
				...
				{
				$mensaje_error = 'Problemas al cambiar el estado de la Cotizaci&oacute;n. Contactese con el administrador';
				}
				else
				{
	...
	general::writeevent('La cotizaci&oacute;n n&uacute;mero '.$_REQUEST['id_cotizacion'].' no puede ser duplicada se encuentra vencida');
	}
	else {
...
general::alert('La Cotizaci&oacute;n n&uacute;mero '.$_REQUEST['id_cotizacion'].' no puede ser duplicada puesto que se encuentra vencida');
}
else{
...
if (!$ListEnc->isvoid()) {
	...
	$MiTemplate->set_var('visible_renta','<tr>
								...
								<td width="200"></td><td width="100" align="left">Retenci�n Renta</td>
	...
	if($visible_reteiva == true){
		$MiTemplate->set_var('visible_reteiva','<tr>
								...
								<td width="100" align="left">Retenci&oacute;n IVA</td>
...
if(!$noacciones){
	...
	if ($id_estado_inicial){
			...
			if ($id_estado=='CV'){
				...
				$generaoe="<input type='button' class='Textonormal' style='width:200;color:#FF0000' name='Button' value='Generar Orden Entrega' onClick=GeneraOe($id_cotizacion)>";
				$MiTemplate->set_var('financia', "<input type='button' class='Textonormal' style='width:200' name='Button' value='Financiaci&oacute;n' onClick=financia($id_cotizacion)>");	
				$editar="<input type='button' class='Textonormal' style='width:200' name='Button' value='Editar' onClick=Editar($id_cotizacion)>";
				$MiTemplate->set_var('editar', $editar);
			}		
		}
...
elseif($id_estado=='CD'){
	$duplicarcotizaciocaducada="<input type='button' class='Textonormal' style='width:200' name='Button' value='Duplicar Cotizaci�n Caducada' onClick=DuplicarCaducada($id_cotizacion)>";
...
$cliente_admitido_CVE = false;
$Listgroup  = new connlist;
bizcve::getcu_group($Listgroup);
$Listgroup->gofirst();
if (!$Listgroup->isvoid()){
	do {
		//general::writeevent("grupos recorridos".$Listgroup->getelem()->id_group);
		if(2==$Listgroup->getelem()->id_group){
			...
			if($Listgroup_id_contribuyente->numelem() > 0){
					...
					for($ic=0; $ic<$Listgroup_id_contribuyente->numelem(); $ic++){
						...
						if($id_contribuyente_cambiocliente_unico==$Listgroup_id_contribuyente->getelem()->id_contribuyente)
						{
							$cliente_admitido_CVE = true;
							break;		
						}
...
if($cliente_admitido_CVE){
        //Mantis 30518: Deshabilitacion boton de generacion de cotizacion inicio 
        // se agrega $btn, que permite que el boton se habilite o no desabilite
	$MiTemplate->set_var('boton_nueva_cotizacion','<input type="button" class="Textonormal" style="width:200" value="Genera Nueva Cotizaci?n"'.$btn.' onClick="NuevaCot({rut__cot} )">');
