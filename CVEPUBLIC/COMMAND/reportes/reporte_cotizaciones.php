	...
	if (!$Listado->isvoid()) {
		...
		do {			
		 	if ($contadorr >= (($pagina-1)*30) && $contadorr <= ((($pagina-1)*30)+30)) { 
				...
				$MiTemplate->set_var("valor13", $Listado->getelem()->codigovendedor);
			 	$MiTemplate->set_var("valor14", $Listado->getelem()->apellidoynombre); 
				...
				$MiTemplate->set_var("valor12", $Listado->getelem()->margenpromedio+0);
                                $MiTemplate->set_var("valor15", ($Listado->getelem()->Guia)?$Listado->getelem()->Guia:'&nbsp;');//Mantis 27362
                                $MiTemplate->set_var("valor16", ($Listado->getelem()->Nro_OP)?$Listado->getelem()->Nro_OP:'&nbsp;');//Mantis 27362
...
if ($_REQUEST['accion'] == 'Excel') {
        ...
        bizcve::reportecotizacionesEX($Listado);
