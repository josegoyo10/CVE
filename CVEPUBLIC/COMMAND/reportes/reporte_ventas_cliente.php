...
$mRegistrousu->id_tipousuario='1,2';
$mRegistrousu->usr_tipo='VE';
$mRegistrousu->usr_estado=" 0";
...
$MiTemplate->set_block('main' , "datosusr" , "BLO_datosusr");
	do {
		$MiTemplate->set_var('nombre_vendedor', $Listusuario->getelem()->usr_nombres.' '.$Listusuario->getelem()->usr_apellidos." (".($Listusuario->getelem()->cod_local?$Listusuario->getelem()->cod_local:'LOCAL NO ASIGNADO').")");
...
$fectermino=$fecterminosiste;
	...
	$mRegistro->tipofolio = $tipofolio;

	$mRegistro_margen=$mRegistro;		
	$Listado->addlast($mRegistro);
	$ListadoMargen->addlast($mRegistro_margen);
	bizcve::getventascliente($Listado);
	bizcve::getventascliente_margen($ListadoMargen);
	$Listado->gofirst();
	$ListadoMargen->gofirst();
	
if ($_REQUEST['accionexport'] == 'exportar') {

header('Location: archivoexcel.php?fecinicio='.$fecinicio.'&fectermino='.$fectermino.'&tipo_cliente='.$tipo_cliente.'&rut='.$rut.'&nomcliente='.$nomcliente.'&codvenusr='.$codvenusr.'&codlocalemi='.$codlocalemi.'&tipofolio='.$tipofolio);
...
$opcion1=$configclitipo->EMPRESARIAL;	
	...
	if (!$Listado->isvoid()) {
		do {
			...
			if ($contadorr > LIMITE_REPORTE_VENTASCLIENTES) {
				...
				$MiTemplate->set_var("margen",number_format($ListadoMargen->getelem()->totalmargen,2));
		...
		} while ($Listado->gonext() && $ListadoMargen->gonext());
	}
	...
	$MiTemplate->set_var("tipofolio",$tipofolio);
