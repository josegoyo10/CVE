...
$MiTemplate->set_file("main", TEMPLATE ."reportes/reporte_facturas.htm");

//Recuperamos y asignamos los parámetros de consulta
/*** Tipo Documento ***/
$List = new connlist;
bizcve::gettipodocumentoreporte($List);
$List->gofirst();
if (!$List->isvoid()) {
...
if ($_REQUEST['accion'] == 'ver') {
	...
	bizcve::getreportefacturas($Listado);
	$Listado->gofirst();
	
	//Cuerpo del reporte
	$MiTemplate->set_file("reportedetalle", TEMPLATE ."reportes/reporte_facturas_detalle.htm");
	...
	if (!$Listado->isvoid()) {
		do {
			++$contadorr;
			if ($contadorr > LIMITE_REPORTE_FACTURAS && !$contadorstat) {
			...
			if (!$contadorstat) {
				...
				$MiTemplate->set_var("valor6", (($Listado->getelem()->nomloccsum)?$Listado->getelem()->nomloccsum:'&nbsp;'));
				$MiTemplate->set_var("valor7", (($Listado->getelem()->nomlocemi)?$Listado->getelem()->nomlocemi:'&nbsp;'));
				...
				$MiTemplate->set_var("valor10",$Listado->getelem()->tipopago);
	...
	if ($contadorr) {
		$MiTemplate->set_var("mensajelimite", "S&oacute;lo se muestran los primeros " .LIMITE_REPORTE_FACTURAS." registros.");
	}
	if (!$contadorr) {
