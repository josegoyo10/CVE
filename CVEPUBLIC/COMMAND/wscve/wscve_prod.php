...
function EnviaOd_cve($cons,&$msg) {
  ...
  //GENERA DETALLE DE RETORNO X CADA PRODUCTO 
	...
	if($L_fac_enc){
		...
		/*entra si el tipo de pago es crÃ©to, si es contado no reserva disponible*/
		if ($idtipopago!=2){
...
function confirmaok_gd($od, &$msgerror){
        ...
        if ($err) {
                // Display the error
                general::writelog("ERROR: en invocación soapclient: $err");
                return 0;
        }
        ...
        if ($client->fault) {
                general::writelog("ERROR: Fault en invocación de rutina confirmaok_gd: $result");
                return 0;
        }
        ...
        if ($err) {
                // Display the error
                general::writelog("ERROR: en invocación de rutina confirmaok_gd: $err");
                return 0;
        }
...
function confirmaerr_gd($od, &$msgerror){
        ...
        if ($err) {
                // Display the error
                general::writelog("ERROR: en invocaciÃ³e soapclient: $err");
                return 0;
        }
        ...
        // chequea error del client
        if ($client->fault) {
                general::writelog("ERROR: Fault en invocaciÃ³e rutina cconfirmaerr_gd $result");
                return 0;
        }
        ...
        if ($err) {
                // Display the error
                //general::writelog("ERROR: en invocaciÃ³e rutina cconfirmaerr_gd $err");
                return 0;
        }
