...
function listado_usuarios( $bd,$patron) {
    ...
    $MiTemplate->set_var("TEXT_TITULO","Administracion de usuarios con Ordenes Bloqueadas ");
    ...
    if( general::se_puede( 'i', PERMISOS_MOD ) )
          $barra_her .= " " . general::kid_href( 'adm_orden_blo_users.php', 'action=ins', $BOTON_AGREGAR_IMG, 'Agregar', '' );
    ...
    // Agregamos el main
    $MiTemplate->set_file("main",TEMPLATE."admOrden_Blo_usuarios/listado_Users_bloq.html");
	...
	while ($row = $res->fetch_assoc()){
		...
		$msg_aux = general::kid_href( 'adm_orden_blo_users.php', "action=upd&usr_id=".$row['usr_id'], $BOTON_MODIFICAR_IMG, 'Modificar', '' ) . " ";
	
		$msg_aux = general::kid_href( 'adm_orden_blo_users.php', "action=upd&usr_id=".$row['usr_id'], $BOTON_MODIFICAR_IMG, 'Modificar', '' ) . " ";
		if( general::se_puede( 'd', PERMISOS_MOD ) )
		$msg_aux .= general::kid_href( 'javascript:validar_eliminar( "'.$CONFIRM_ELIMINAR_USR.'", "adm_orden_blo_users.php?action=del&usr_id='.$row['usr_id'].'" ); //', '', $BOTON_ELIMINAR_IMG, 'Eliminar', '' );
