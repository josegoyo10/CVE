...
tinyMCE.importPluginLanguagePack('flash', 'en,de,pl');
...
function TinyMCE_flash_execCommand(editor_id, element, command, user_interface, value) {
    ...
    switch (command) {
        case "mceFlash":
			var name = "", swffile = "", swfwidth = "", swfheight = "", swfalign = "", action = "insert";
            var template = new Array();
			var inst = tinyMCE.getInstanceById(editor_id);
			var focusElm = inst.getFocusElement();

            template['file']   = '../../plugins/flash/InsertFlash/insert_flash.php'; // Relative to theme
            template['width']  = 600;
            template['height'] = 400;
            ...
            if (focusElm != null && focusElm.nodeName.toLowerCase() == "img") {
				...
				swfalign = getAttrib(focusElm, 'align');
				action = "update";
            }

            tinyMCE.openWindow(template, {editor_id : editor_id, swffile : swffile, swfwidth : swfwidth, swfheight : swfheight, swfalign : swfalign, action : action});
		return true;
   }
	...
	switch (type) {
		case "insert_to_editor":
			...
			while ((startPos = content.indexOf('<object', startPos)) != -1) {
				...
				content += '<img name="mce_plugin_flash" width="' + attribs["width"] + '" height="' + attribs["height"] + '" align="' + attribs["align"] + '"';
		...
		case "get_from_editor":
			...
			while ((startPos = content.indexOf('<img', startPos)) != -1) {
				...
				embedHTML += ' width="' + attribs["width"] + '" height="' + attribs["height"] + '" align="' + attribs["align"] + '">';
                ...
                embedHTML += '<param name="movie" value="' + attribs["title"] + '" />';
                embedHTML += '<embed src="' + attribs["title"] + '" quality="high" menu="false" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="' + attribs["width"] + '" height="' + attribs["height"] + '" align="' + attribs["align"] + '"></embed></object>';
