...
function createFolder() {
        ...
        global $MY_PATH;
        if (!$MY_ALLOW_CREATE) return (_swf_nopermtocreatefolder);
        if (!(is_dir($MY_DOCUMENT_ROOT.$MY_PATH))) return (_swf_pathnotfound);
        if ( !isset($_REQUEST['file'])) return (_swf_foldernamemissing);
        $Folder = checkName($_REQUEST['file']);
        //$Folder = utf8RawUrlDecode($Folder);
        $newFolder = $MY_DOCUMENT_ROOT.$MY_PATH.$Folder;
        if (is_dir($newFolder)) return (_swf_folderalreadyexists);
        $newFolder = unsanitize($newFolder);
        if (!(@mkdir($newFolder,0755))) return (_swf_mkdirfailed);
...
function deleteFile() {
        ...
        if (!$MY_ALLOW_DELETE) return (_swf_nopermtodelete);
        ...
        if (isset($_REQUEST['files']) && is_array($_REQUEST['files'])) {
            foreach ($_REQUEST['files'] as $file) {
                        ...
                        if (is_file($delFile)) {
                                if (!(unlink($delFile))) $error = $error.'\n'.alertSanitize(_swf_unlinkfailed.' ('.$delFile.')');
                        } else {
                                $error = $error.'\n'.alertSanitize(_swf_filenotfound.' ('.$delFile.')');
                        }
            }
        }
        $refresh_dirs = true;
...
function renameFile() {
        ...
        $error = false;
        if (!$MY_ALLOW_RENAME) return (_swf_nopermtorename);
        if (isset($_REQUEST['folders']) && is_array($_REQUEST['folders'])) {
            foreach ($_REQUEST['folders'] as $file) {
                        ...
                        if (is_dir($oldFile)) {
                                if (is_dir($newFile)) {
                                        $error = $error.'\n'.alertSanitize(_swf_folderalreadyexists.' ('.$oldFile.' -> '.$newFile.')');
                                } else {
                                        if (!rename($oldFile, $newFile)) $error = $error.'\n'.alertSanitize(_swf_renamefailed.' ('.$oldFile.' -> '.$newFile.')');
                                }
                        } else {
                                $error = $error.'\n'.alertSanitize(_swf_foldernotfound.' ('.$oldFile.')');
                        }
            }
         }
        ...
        if (isset($_REQUEST['files']) && is_array($_REQUEST['files'])) {
            foreach ($_REQUEST['files'] as $file) {
                        ...
                        if (is_file($oldFile)) {
                                if (is_file($newFile)) {
                                        $error = $error.'\n'.alertSanitize(_swf_filealreadyexists.' ('.$oldFile.' -> '.$newFile.')');
                                } else {
                                        if (!rename($oldFile, $newFile)) $error = $error.'\n'.alertSanitize(_swf_renamefailed.' ('.$oldFile.' -> '.$newFile.')');
                                }
                    } else {
                                $error = $error.'\n'.alertSanitize(_swf_filenotfound.' ('.$oldFile.')');
                        }
            }
         }
...
function moveFile() {
        ...
        global $MY_ALLOW_EXTENSIONS, $MY_DENY_EXTENSIONS ;
        $error = false;
        if (!$MY_ALLOW_MOVE) return (_swf_nopermtomove);
        $newPath = pathSlashes(checkName($_REQUEST['newpath']));
         if (!(is_dir($MY_DOCUMENT_ROOT.$newPath))) return (_swf_pathnotfound);
        if (isset($_REQUEST['folders']) && is_array($_REQUEST['folders'])) {
            foreach ($_REQUEST['folders'] as $file) {
...
function uploadFile() {
        ...
        if (!$MY_ALLOW_UPLOAD) return (_swf_nopermtoupload);
        if (!(is_dir($MY_DOCUMENT_ROOT.$MY_PATH))) return (_swf_pathnotfound);
        $filename = checkName($_FILES['uploadFile']['name']);
        $newFile = $MY_DOCUMENT_ROOT.$MY_PATH.$filename;
        $parts = explode('.', $filename);
        $ext = strtolower($parts[count($parts)-1]);
        if (is_file($newFile))  return (_swf_uploadfilealreadyexists);
        if (is_array($MY_DENY_EXTENSIONS )) {
                if (in_array($ext, $MY_DENY_EXTENSIONS)) return (_swf_extnotallowed);
        }
        if (is_array($MY_ALLOW_EXTENSIONS )) {
                if (!in_array($ext, $MY_ALLOW_EXTENSIONS)) return (_swf_extnotallowed);
        }
        if ($MY_MAX_FILE_SIZE) {
                if ($_FILES['uploadFile']['size'] > $MY_MAX_FILE_SIZE) return (_swf_filesizeexceedlimit.' of '.($MY_MAX_FILE_SIZE/1024).'kB.');
        }
        if (!is_file($_FILES['uploadFile']['tmp_name']))  return (_swf_filenotuploaded);
...
function draw_no_dir() {
        ...
        echo _swf_configproblem." ".$MY_DOCUMENT_ROOT;
        echo '</div></td></tr></table>';
}
...
function fileSelected(filename, width, height) {
        var topDoc = window.top.document.forms[0];
        topDoc.file.value = filename;
        topDoc.swf_width.value = width;
        topDoc.swf_height.value = height;
        topDoc.original_width.value = width;
        topDoc.original_height.value = height;
...
if($d) {
        ...
        <thead>
                <tr>
                        <td width="4%" id="sortmefirst" onclick="setSortBy(0, false);">'._swf_type.'</td>
                        <td width="50%" title="CaseInsensitiveString" onclick="setSortBy(1, false);">'._swf_name.'</td>
                        <td width="13%" onclick="setSortBy(2, false);">'._swf_size.'</td>
                        <td width="25%" onclick="setSortBy(3, false);">'._swf_datemodified.'</td>
                </tr>
        </thead>
        ...
        while (false !== ($entry = $d->read())) {
                if(substr($entry,0,1) != '.') {
                        ...
                        if (is_dir($absolutePath)) {
                                ...
                                <td width="18%" align="right">'._swf_folder.'</td>
                                <td width="25%">'.$parsed_time.'</td>
                                <td width="0px" style="display: none;">&nbsp;</td>
                                <td width="0px" style="display: none;">&nbsp;</td>
                            <td width="0px" style="display: none;">'.$time.'</td>
                                </tr>';
                                $folderJSArray .= "['images/ext/folder_small.gif', '".sanitize($entry)."', '"._swf_folder."', '".$parsed_time."'],\n";
                $folderNb++;
                        } else {
                                $entries_cnt++;
                                ...
                                $parsed_icon = 'img/ext/'.parse_icon($ext);
                                                                list ($f_width, $f_height) = getimagesize($absolutePath);
                                $t_files .= '<tr id="F'.$fileNb++.'">
                                <td width="4%"><img src="'.$parsed_icon.'" width="16" height="16" border="0" alt="'.$entry.'" /></td>
                                <td width="50%"><div style="height:15px; overflow:hidden;"><a href="javascript:;" onClick="javascript:fileSelected(\''.$MY_BASE_URL.$relativePath.'\',\''.$f_width.'\',\''.$f_height.'\')">'.$entry.'</div></td>
