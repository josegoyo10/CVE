...
class ImageManager
{
     ...
     * @return string tmp file prefix.
        ...
        function getThumbURL($relative)
        {
                ...
                if($path_parts['dirname']=='\\') $path_parts['dirname']='/';
                 ...
                 if($this->config['safe_mode']
                        || strlen(trim($this->config['thumbnail_dir'])) == 0)
                {
                        $path = Files::makePath($this->getBaseURL(),$path_parts['dirname']);
                        Return Files::makeFile($path,$thumbnail);
                }
                else
                {
        ...
        /**
         *Check for valid image type. We only allow jpg/jpeg, gif and png
        */
        ...
        function processUploads()
        {
                ...
                //Added some $_POST variables for upload resizing - see _processFiles - Ryan Demmer Feb 2005
                ...
                {
                        $this->_processFiles($relative, $_FILES['upload'], $resize, $max_width, $max_height);
                }
        }
         ...
         * false otherwise
         */
        function _processFiles($relative, $file, $resize, $max_width, $max_height)
        {
                if($file['error']!=0)
                {
                        Return false;
                ...
                if($this->config['validate_images'] == true && !$this->validImageType($file['tmp_name']))
                {
                        //$imgInfo = @getImageSize($file['tmp_name']);
                        //if(!is_array($imgInfo))
                        //{
                        //Added image check. We only want valid types to be upload. Ryan Demmer March 2005
                                Files::delFile($file['tmp_name']);
                                echo "<script>javascript: alert('Only JPEG, PNG and GIF images are supported.');</script>";
                                Return false;
                        //}
                }
                ...
                //Added image resizing - Ryan Demmer Feb 2005
                ...
                if ($resize == true && $validtype && ($imgInfo[0] > $max_width
                 || $imgInfo[1] > $max_height)){

                    //resize image
                    $resize = new Resize($max_width,$max_height);
                    $resize->resizeImage($file['tmp_name'], $type, $quality);
                }

                $result = Files::copyFile($file['tmp_name'], $path, $file['name']);

                //Create Thumbnail
                //Added by Ryan Demmer 21/02/2005
                if($validtype && $this->config['thumbs'] == 'true'){
                    $fullpath = Files::makeFile($this->getBaseDir(),$result);

                    $thumbnail = $this->getThumbName($fullpath);

                    $thumbnailer = new Thumbnail($this->config['thumbnail_width'],$this->config['thumbnail_height']);
        ...
        function getThumbnail($relative)
        {
                ...
                $validtype = $this->validImageType($fullpath);
                ...
                if($imgInfo[2] == '1' && $this->isGDGIFAble() != 1 && !file_exists($thumbnail)){
                    Return $this->getDefaultThumb();
                }
                //not an image
                //if(!is_array($imgInfo))
                ...
                //Create Thumbnail Added by Ryan Demmmer 21/02/2005
                //Check whether the image is a valid type - gif, jpeg, png
                if ($validtype && $this->config['thumbs'] == 'true'){
                    $thumbnailer = new Thumbnail($this->config['thumbnail_width'],$this->config['thumbnail_height']);
                    $thumbnailer->createThumbnail($fullpath, $thumbnail);
                }
