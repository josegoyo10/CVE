...
require($mosConfig_absolute_path.'/mambots/editors/tinymce_exp/jscripts/tiny_mce/plugins/filemanager/InsertFile/config.inc.php');
include ($mosConfig_absolute_path."/mambots/editors/tinymce_exp/jscripts/tiny_mce/plugins/filemanager/langs/$editor_lang.php");
require($mosConfig_absolute_path.'/mambots/editors/tinymce_exp/jscripts/tiny_mce/plugins/filemanager/InsertFile/functions.php');
...
function createFolder() {
        ...
        global $MY_PATH;
        if (!$MY_ALLOW_CREATE) return (_filemanager_nopermtocreatefolder);
        if (!(is_dir($MY_DOCUMENT_ROOT.$MY_PATH))) return (_filemanager_pathnotfound);
        if ( !isset($_REQUEST['file'])) return (_filemanager_foldernamemissing);
        $Folder = checkName($_REQUEST['file']);
        //$Folder = utf8RawUrlDecode($Folder);
        $newFolder = $MY_DOCUMENT_ROOT.$MY_PATH.$Folder;
        if (is_dir($newFolder)) return (_filemanager_folderalreadyexists);
        $newFolder = unsanitize($newFolder);
        if (!(@mkdir($newFolder,0755))) return (_filemanager_mkdirfailed);
...
function deleteFile() {
        ...
        if (!$MY_ALLOW_DELETE) return (_filemanager_nopermtodelete);
        ...
        if (isset($_REQUEST['files']) && is_array($_REQUEST['files'])) {
            foreach ($_REQUEST['files'] as $file) {
                        ...
                        if (is_file($delFile)) {
                                if (!(unlink($delFile))) $error = $error.'\n'.alertSanitize(_filemanager_unlinkfailed.' ('.$delFile.')');
                        } else {
                                $error = $error.'\n'.alertSanitize(_filemanager_filenotfound.' ('.$delFile.')');
                        }
            }
        }
        $refresh_dirs = true;
...
function renameFile() {
        ...
        $error = false;
        if (!$MY_ALLOW_RENAME) return (_filemanager_nopermtorename);
        if (isset($_REQUEST['folders']) && is_array($_REQUEST['folders'])) {
...
function moveFile() {
        ...
        global $MY_ALLOW_EXTENSIONS, $MY_DENY_EXTENSIONS ;
        $error = false;
        if (!$MY_ALLOW_MOVE) return (_filemanager_nopermtomove);
        $newPath = pathSlashes(checkName($_REQUEST['newpath']));
         if (!(is_dir($MY_DOCUMENT_ROOT.$newPath))) return (_filemanager_pathnotfound);
        if (isset($_REQUEST['folders']) && is_array($_REQUEST['folders'])) {
            foreach ($_REQUEST['folders'] as $file) {
...
function uploadFile() {
        ...
        if (!$MY_ALLOW_UPLOAD) return (_filemanager_nopermtoupload);
        if (!(is_dir($MY_DOCUMENT_ROOT.$MY_PATH))) return (_filemanager_pathnotfound);
        $filename = checkName($_FILES['uploadFile']['name']);
        $newFile = $MY_DOCUMENT_ROOT.$MY_PATH.$filename;
        $parts = explode('.', $filename);
        $ext = strtolower($parts[count($parts)-1]);
        if (is_file($newFile))  return (_filemanager_uploadfilealreadyexists);
        if (is_array($MY_DENY_EXTENSIONS )) {
                if (in_array($ext, $MY_DENY_EXTENSIONS)) return (_filemanager_extnotallowed);
        }
        if (is_array($MY_ALLOW_EXTENSIONS )) {
                if (!in_array($ext, $MY_ALLOW_EXTENSIONS)) return (_filemanager_extnotallowed);
        }
        if ($MY_MAX_FILE_SIZE) {
                if ($_FILES['uploadFile']['size'] > $MY_MAX_FILE_SIZE) return (_filemanager_filesizeexceedlimit.' of '.($MY_MAX_FILE_SIZE/1024).'kB.');
        }
        if (!is_file($_FILES['uploadFile']['tmp_name']))  return (_filemanager_filenotuploaded);
...
function draw_no_dir() {
        ...
        echo _filemanager_configproblem." ".$MY_DOCUMENT_ROOT;
        echo '</div></td></tr></table>';
}
...
<html>
        <head>
                ...
                <?php
                        echo '<meta http-equiv="content-language" content="'.$editor_lang.'" />'."\n";
...
if($d) {
        ...
        <thead>
                <tr>
                        <td width="4%" id="sortmefirst" onclick="setSortBy(0, false);">'._filemanager_type.'</td>
                        <td width="50%" title="CaseInsensitiveString" onclick="setSortBy(1, false);">'._filemanager_name.'</td>
                        <td width="13%" onclick="setSortBy(2, false);">'._filemanager_size.'</td>
                        <td width="25%" onclick="setSortBy(3, false);">'._filemanager_datemodified.'</td>
                </tr>
        </thead>
