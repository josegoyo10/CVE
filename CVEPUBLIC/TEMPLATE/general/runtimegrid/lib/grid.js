...
element.attachEvent=function(name,handler){
	...
	var nsHandler=function(event){window.event=event;handler();window.event=null};
...
var getClientWidth=function(){return this.offsetWidth - 20};
var getClientHeight=function(){return this.offsetHeight - 20};
element.__defineGetter__("clientWidth",getClientWidth);
element.__defineGetter__("clientHeight",getClientHeight);
var getRuntimeStyle=function(){return this.style};
...
var getCurrentStyle=function(){return document.defaultView.getComputedStyle(this,"")};
...
(function(){
	...
	if(!event){return}
	var getSrcElement=function(){return(this.target.nodeType==3)?this.target.parentNode:this.target};
	event.__defineGetter__("srcElement",getSrcElement);
	var setReturnValue=function(value){if(!value){this.preventDefault()}};
...
(function(){
	...
	stylesheet.addRule=function(selector,rule){this.insertRule(selector+"{"+rule+"}",this.cssRules.length)};
	stylesheet.__defineGetter__("rules",function(){return this.cssRules})
})();
(function(){
	if(!window.XMLHttpRequest){return}
	var ActiveXObject=function(type){ActiveXObject[type](this)};
	ActiveXObject["MSXML2.DOMDocument"]=function(obj){obj.setProperty=function(){};
	...
	ActiveXObject["MSXML2.XMLHTTP"]=function(obj){
		obj.open=function(method,url,async){
			...
			obj.send=function(data){this.request.send(data)};
			obj.setRequestHeader=function(name,value){this.request.setRequestHeader(name,value)};
...
(function(){
	...
	doc.setProperty=function(name,value){
		if(name=="SelectionNamespaces"){namespaces={};
		var a=value.split(" xmlns:");
		for(var i=1;i<a.length;i++){
			var s=a[i].split("=");
			namespaces[s[0]]=s[1].replace(/\"/g,"")}this._ns={lookupNamespaceURI:
			function(prefix){
...
doc._ns={lookupNamespaceURI:function(){return null}};
...
doc.selectSingleNode=function(path){return xpath.evaluate(path,this,this._ns,9,null).singleNodeValue};
...
element.selectSingleNode=function(path){return xpath.evaluate(path,this,this.ownerDocument._ns,9,null).singleNodeValue};
...
Active.System.Object.handle=function(error){throw(error)};
Active.System.Object.create=function(){
	var obj=this.prototype;obj.clone=function(){
	...
	obj._clone=function(){};
	obj.init=function(){};
	obj.handle=function(error){throw(error)};
	...
	Active.System.Model.create=function(){
		...
		obj.defineProperty=function(name,value){
			...
			if(typeof value=="function"){
				...
				Active.System.Model.create(); Active.System.Format=Active.System.Object.subclass();
				Active.System.Format.create=function(){
					var obj=this.prototype;
					obj.valueToText=function(value){return value};
					obj.dataToValue=function(data){return data};
 ...
 Active.Templates.Link.create=function(){var obj=this.prototype;obj.setTag("a");obj.setClass("templates","link");obj.setAttribute("href",function(){return this.getItemProperty("link")});var image=new Active.HTML.SPAN;image.setClass("box","image");image.setClass("image",function(){return this.getItemProperty("image")});obj.setContent("image",image);obj.setContent("text",function(){return this.getItemProperty("text")});
 
 obj.setEvent("onclick",function(){this.action("click")})};
 Active.Templates.Link.create(); 
 Active.Templates.Item=Active.System.Template.subclass();
 Active.Templates.Item.create=function(){var obj=this.prototype;obj.setClass("templates","item");obj.setClass("box","normal");var box=new Active.HTML.DIV;
 var image=new Active.HTML.SPAN;box.setClass("box","item");
...
obj.defineStatusProperty("text",function(){
	switch(this.getStatusProperty("code")){
		case "loading":return "Cargando los datos, por favor espere...";
 		case "nodata":return "No hay productos para desplegar";
 		//case "nodata":return "Seleccione una categoría del árbol de productos<br>(Sólo pueden seleccionarse categorías terminales)";
 		default:return ""
	}
});
...
obj.defineStatusProperty("image",function(){
	...
	obj.setRowHeaderWidth=function(width){
		...
		var startColumnResize=function(header){var el=header.element();
		var pos=event.clientX;
		var size=el.offsetWidth;var grid=this;
		...
		var endResize=function(){
			...
			el.releaseCapture();
			var width=size+event.clientX - pos;if(width < 5){width=5}
			...
			el=null;grid.getTemplate("layout").action("adjustSize")};
