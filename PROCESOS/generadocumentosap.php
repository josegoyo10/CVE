...
/****************************************** MODIFICACION GOA 19-12-2007. Marca en detalle para producto flete  ************************************************/						
			if ($listaDet->getelem()->codprod == 12501){
				...
				$detalle .= general::CompletaEspaciosD('D', 1);
															//TIP_REG		CHAR	1	0	Tipo de Registro
			}			
...
function creaMsgode($listaEnc,$listaDet,$odea){
			...
			//Obtengo información de oe de cotizacion
			$oe = new ctrlordenent();
			$oe->getordenent($ListEnc = new connlist(new dtoencordenent(array('id_ordenent'=>$listaEnc->getelem()->id_ordenent))));
			$ListEnc->gofirst();
			$cotizacion = $ListEnc->getelem()->id_cotizacion;
	
			//Obtengo información de flete de cotizacion
			$coti = new ctrlcotizacion();
			$coti->getcotizacionsap($ListDet = new connlist(new dtodetcotizacion(array('id_cotizacion'=>$cotizacion))));
			$ListDet->gofirst();
			$costoflete = round($ListDet->getelem()->cargoflete);

			//Obtengo datos de precio venta neto para la cotizacion
			$pventa = new ctrlcotizacion();
			$pventa->getpventanetosap($ListsapDet = new connlist(new dtodetcotizacion(array('id_cotizacion'=>$cotizacion))));
			$ListsapDet->gofirst();
			...
			if($costoflete){
				$encabezado .= general::CompletaCerosI($costoflete, 15); 							//ZFLT	DEC	15	2	Valor neto total del flete.			
			}
			/*MODIFICACION MVE 20 MARZO 2008. Se agrega el valor neto total del flete.*/
			//$encabezado .= general::CompletaEspaciosD($listaEnc->getelem()->giro, 35); 			//BRSCH_TEXT	CHAR	20	0	Descripcion del giro
			$encabezado .= "\n";
	...
	/****************************************** MODIFICACION GOA 19-12-2007. Marca en detalle para producto flete  ************************************************/						
						...
						$detalle .= general::CompletaCerosI($cantsd, 11).'.'.general::CompletaCerosD($cantde, 3); 	//REQ_QTY	DEC		15	3	Cantidad
						$detalle .= general::CompletaEspaciosD(general::umconvertcvesap($listaDet->getelem()->unimed), 3); 					//SALES_UNIT	CHAR	3	0	Unidad de medida de venta
						//Obtengo información de pventaneto de cotizacion
						$detalle .= general::CompletaCerosI(round($ListsapDet->getelem()->pventaneto), 12).'.00'; 	//PRECIO_VENTA	DEC	15	2	Precio de venta
						$detalle .= general::CompletaCerosI(round($listaDet->getelem()->pcosto), 12).'.00'; 		//TODO: 2da etapa OE con PCosto //PRECIO_COSTO	DEC	15	2	Precio de costo
						$detalle .= "\n";
					} while ($listaDet->gonext()&& $ListsapDet->gonext());
			}
